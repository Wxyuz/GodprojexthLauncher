...
$PreferredAssetName = "FreedxmLauncher_LoginSample.zip"
$Sha256 = "E704BDDF50DFB48DE8D6B657E047555EED0ADE18D886BF4405EF3A84631D3155"
...
function Set-LoaderState($status, $percent) {
    ...
    if ($script:StatusLabel -ne $null) { $script:StatusLabel.Text = $script:CurrentStatus }
    if ($script:PercentLabel -ne $null) { $script:PercentLabel.Text = "$($script:CurrentPercent)%" }
    if ($script:ProgressBar -ne $null) { $script:ProgressBar.Value = $script:CurrentPercent }
...
function Start-InstallerJob {
    try {
        ...
        $Headers = @{ "User-Agent" = "FreedxmLauncherInstaller" }
        ...
        $Vbs = Get-ChildItem -Path $InstallDir -Filter "Launch_FreedxmLauncher.vbs" -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 1
        $Bat = Get-ChildItem -Path $InstallDir -Filter "run.bat" -Recurse -File -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($Vbs) { Start-Process -FilePath $Vbs.FullName }
        elseif ($Ps1) { Start-Process powershell -WindowStyle Hidden -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$($Ps1.FullName)`"" }
        elseif ($Bat) { Start-Process -FilePath $Bat.FullName -WorkingDirectory $Bat.DirectoryName }
        else { Start-Process explorer.exe $InstallDir }
...
$CloseButton.Add_Click({ $LoaderForm.Close() })

$AnimationTimer = New-Object System.Windows.Forms.Timer
$AnimationTimer.Interval = 120
$AnimationTimer.Add_Tick({
    if ($script:LoaderStep -ge 3) { $script:LoaderStep = 0 } else { $script:LoaderStep++ }
